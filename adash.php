...
// Database connection
 ...
 $username = "root";
 $password = "";
 $dbname = "college_feedback_system"; // Updated to match your database
...
// Get admin details
 ...
 $sql = "SELECT adminid, email, department_id FROM admin_users WHERE adminid = '$admin_id'";
...
if ($result->num_rows > 0) {
    ...
    $dept_sql = "SELECT name FROM departments WHERE department_id = '$dept_id'";
    $dept_result = $conn->query($dept_sql);
    if ($dept_result->num_rows > 0) {
        $dept_row = $dept_result->fetch_assoc();
        $admin['branch'] = $dept_row['name'];
    } else {
...
// Get statistics from database
 ...
 $totalClasses = 0;
 $totalMeetings = 0;
 $totalCourseOutcomes = 0;

// Get total teachers (faculty) - all teachers, not just active ones
 $teachersQuery = "SELECT COUNT(*) as count FROM faculty";
 $teachersResult = $conn->query($teachersQuery);
if ($teachersResult && $teachersResult->num_rows > 0) {
    $row = $teachersResult->fetch_assoc();
    $totalTeachers = $row['count'];
}

// Get total classes (subjects)
 $classesQuery = "SELECT COUNT(*) as count FROM subjects WHERE is_active = 1";
 $classesResult = $conn->query($classesQuery);
if ($classesResult && $classesResult->num_rows > 0) {
    $row = $classesResult->fetch_assoc();
    $totalClasses = $row['count'];
}

// For meetings, since there's no explicit meetings table in your schema, 
// we'll use a placeholder value or you can modify this query based on your actual meetings table
 $totalMeetings = 0; // Placeholder - update with your actual meetings query

// Get total course outcomes
 $courseOutcomesQuery = "SELECT COUNT(*) as count FROM course_outcomes";
 $courseOutcomesResult = $conn->query($courseOutcomesQuery);
if ($courseOutcomesResult && $courseOutcomesResult->num_rows > 0) {
    $row = $courseOutcomesResult->fetch_assoc();
    $totalCourseOutcomes = $row['count'];
}
...
<body class="antialiased relative bg-background-light dark:bg-background-dark font-display">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between py-6">
                    ...
                    <div class="flex items-center">
                        ...
                        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">College Management</h1>
                    </div>
                    ...
                    <nav class="hidden md:flex space-x-1">
                        ...
                        <a class="flex items-center gap-2 px-4 py-3 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-primary/5 dark:hover:bg-primary/10 transition-all" href="meetings.php">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <a href="admin.php" class="logout-btn flex items-center gap-2 px-4 py-2 rounded-lg text-gray-600 dark:text-gray-300 transition-all">
                ...
                <div class="md:hidden py-4 border-t border-gray-200 dark:border-gray-700">
                    <nav class="flex space-x-1 overflow-x-auto pb-2">
                        ...
                        <a class="flex items-center gap-2 px-4 py-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-primary/5 dark:hover:bg-primary/10 transition-all whitespace-nowrap" href="meetings.php">
                            <i class="fas fa-video text-lg"></i>
                            <span class="font-medium">Meetings</span>
                        </a>
                        <a href="admin.php" class="logout-btn flex items-center gap-2 px-4 py-2 rounded-lg text-gray-600 dark:text-gray-300 transition-all whitespace-nowrap">
                            <i class="fas fa-sign-out-alt text-lg"></i>
                            <span class="font-medium">Logout</span>
                        </a>
                    </nav>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <header class="mb-8 animate-slide-up">
                    <div>
                        ...
                        <p class="text-gray-500 dark:text-gray-400 mt-1">Efficiently manage teachers, classes, meetings, and schedules.</p>
                    </div>
                </header>
                        ...
                        <div class="text-center md:text-left flex-1">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <p class="text-xl font-bold text-gray-800 dark:text-white"><?php echo htmlspecialchars($admin['adminid']); ?></p>
                                </div>
                                <div>
                                ...
                                <div>
                                    <p class="text-gray-500 dark:text-gray-400 text-sm">Department</p>
                ...
                <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
                    <div class="stats-card p-6 animate-slide-up">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-800 dark:text-white mt-2"><?php echo $totalTeachers; ?></p>
                            </div>
                    ...
                    <div class="stats-card p-6 animate-slide-up">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-gray-800 dark:text-white mt-2"><?php echo $totalClasses; ?></p>
                            </div>
                    ...
                    <div class="stats-card p-6 animate-slide-up">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-500 dark:text-gray-400">Scheduled Meetings</p>
                                <p class="text-3xl font-bold text-gray-800 dark:text-white mt-2"><?php echo $totalMeetings; ?></p>
                            </div>
                            <div class="bg-warning/20 p-3 rounded-lg">
                                <i class="fas fa-video text-warning text-2xl"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="stats-card p-6 animate-slide-up">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-500 dark:text-gray-400">Course Outcomes</p>
                                <p class="text-3xl font-bold text-gray-800 dark:text-white mt-2"><?php echo $totalCourseOutcomes; ?></p>
                            </div>
                ...
                <!-- Quick Actions -->
                <section class="mt-8 bg-card-light dark:bg-card-dark p-6 rounded-xl shadow-card animate-slide-up">
                    <h2 class="text-2xl font-semibold text-gray-800 dark:text-white mb-6">Quick Actions</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <a href="teacher.php" class="btn-primary px-6 py-3 rounded-lg text-white font-semibold flex items-center justify-center gap-2 transition-all">
                            <i class="fas fa-user-plus"></i>
                            Add Teacher
                        </a>
                        <a href="classes.php" class="btn-primary px-6 py-3 rounded-lg text-white font-semibold flex items-center justify-center gap-2 transition-all">
                            <i class="fas fa-plus-circle"></i>
                            Add Class
                        </a>
                        <a href="meetings.php" class="btn-primary px-6 py-3 rounded-lg text-white font-semibold flex items-center justify-center gap-2 transition-all">
    ...
    <script>
        ...
        // Logout confirmation - Fixed to only apply to actual logout buttons
        document.addEventListener('DOMContentLoaded', function() {
            ...
            Toast.fire({
                icon: 'success',
                title: 'Welcome, <?php echo htmlspecialchars($admin['adminid']); ?>!'
            });
        });
    </script>
</body>
