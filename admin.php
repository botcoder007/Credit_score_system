...
function sendPasswordEmail($to_email, $admin_name, $admin_id, $password) {
    ...
    try {
        ...
        $mail->addAddress($to_email, $admin_name);
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p>You have requested a password recovery for your admin account. Here are your login credentials:</p>
                    
                    <div class='credentials'>
                        <h3 style='color: #4f46e5; margin-top: 0;'>📋 Your Login Details:</h3>
                        <table style='width: 100%; border-collapse: collapse;'>
                            <tr>
                                <td style='padding: 8px 0; font-weight: bold; color: #555;'>Admin ID:</td>
                                <td style='padding: 8px 0; color: #333; font-size: 16px;'>$admin_id</td>
                            </tr>
                            <tr>
                    ...
                    <div class='login-info'>
                        ...
                        <ol style='margin: 10px 0; padding-left: 20px;'>
                            <li>Go to the Credit Update System admin login page</li>
                            <li>Enter your Admin ID: <strong>$admin_id</strong></li>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['forgot_password'])) {
        $admin_id = trim($_POST['forgot-admin-id']);
        $email = trim($_POST['forgot-email']);
        
        if (empty($admin_id) || empty($email)) {
            $error_message = "Please enter both Admin ID and Email address.";
            $forgot_password_mode = true;
        } else {
            // Check if admin exists with matching email
            $stmt = $conn->prepare("SELECT admin_id, password, email, branch FROM adminc WHERE admin_id = ? AND email = ?");
            ...
            if ($result->num_rows == 1) {
                ...
                if (sendPasswordEmail($email, "Admin", $admin_id, $row['password'])) {
                ...
                $error_message = "Admin ID and Email combination not found in our records. Please verify your details.";
                $forgot_password_mode = true;
            }
    ...
    // Handle regular login
        ...
        error_log("Login attempt - Admin ID: " . $admin_id . ", Password: " . $password . ", Branch: " . $branch);
        
        // Validate input
        if (empty($admin_id) || empty($password) || empty($branch)) {
            $error_message = "Please enter all required fields.";
        } else {
            // Prepare and execute query - using string parameter
            $stmt = $conn->prepare("SELECT admin_id, password, email, branch FROM adminc WHERE admin_id = ? AND branch = ?");
            $stmt->bind_param("ss", $admin_id, $branch);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            if ($result->num_rows == 1) {
                ...
                error_log("Database result - Admin ID: " . $row['admin_id'] . ", Password: " . $row['password'] . ", Branch: " . $row['branch']);
                ...
                if (trim($password) === trim($row['password'])) {
                    ...
                    $error_message = "Invalid Admin ID, Password, or Branch.";
                }
            } else {
                $error_message = "Invalid Admin ID, Password, or Branch. (Admin not found)";
            }
...
<head>
    ...
    <title>Credit Update System - Admin Login</title>
...
<body class="antialiased relative bg-background-light dark:bg-background-dark font-display text-gray-800 dark:text-gray-200">
    ...
    <div class="flex min-h-screen flex-col items-center justify-center p-4">
        <div class="w-full max-w-md">
            <div class="flex flex-col items-center mb-8 animate-fade-in">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Welcome back! Please enter your admin credentials.</p>
            </div>
            ...
            <div class="bg-card-light dark:bg-card-dark p-8 rounded-xl shadow-card hover-card animate-slide-up">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="space-y-6" method="POST">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="admin-id">Admin ID</label>
                        <div class="mt-1 relative">
                            <input class="custom-input block w-full appearance-none rounded-lg border border-gray-300 bg-gray-50 dark:bg-gray-700/50 px-4 py-3 pl-10 text-gray-900 dark:text-white placeholder-gray-400 shadow-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary transition-all" 
                                   ...
                                   required 
                                   type="text"
                                   value="<?php echo isset($_POST['admin-id']) ? htmlspecialchars($_POST['admin-id']) : ''; ?>"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="branch">Branch</label>
                        <div class="mt-1 relative">
                            <select class="custom-select custom-input block w-full appearance-none rounded-lg border border-gray-300 bg-gray-50 dark:bg-gray-700/50 px-4 py-3 pl-10 text-gray-900 dark:text-white placeholder-gray-400 shadow-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary transition-all" 
                                    id="branch" 
                                    name="branch" 
                                    required>
                                ...
                                <option value="Computer Science and Engineering" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'Computer Science and Engineering') ? 'selected' : ''; ?>>Computer Science and Engineering</option>
                                <option value="CSE – Artificial Intelligence & Machine Learning" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'CSE – Artificial Intelligence & Machine Learning') ? 'selected' : ''; ?>>CSE – Artificial Intelligence & Machine Learning</option>
                                <option value="CSE – Data Science" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'CSE – Data Science') ? 'selected' : ''; ?>>CSE – Data Science</option>
                                <option value="CSE – Cyber Security" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'CSE – Cyber Security') ? 'selected' : ''; ?>>CSE – Cyber Security</option>
                                <option value="Electronics and Communication Engineering" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'Electronics and Communication Engineering') ? 'selected' : ''; ?>>Electronics and Communication Engineering</option>
                                <option value="Electrical and Electronics Engineering" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'Electrical and Electronics Engineering') ? 'selected' : ''; ?>>Electrical and Electronics Engineering</option>
                                ...
                                <option value="Biotechnology" <?php echo (isset($_POST['branch']) && $_POST['branch'] == 'Biotechnology') ? 'selected' : ''; ?>>Biotechnology</option>
                            </select>
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
    ...
    <div id="forgotPasswordModal" class="modal-overlay <?php echo $forgot_password_mode ? 'active' : ''; ?>">
        <div class="modal-content">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2" for="forgot-admin-id">Admin ID</label>
                    <div class="relative">
                        <input class="custom-input block w-full appearance-none rounded-lg border border-gray-300 bg-gray-50 dark:bg-gray-700/50 px-4 py-3 pl-10 text-gray-900 dark:text-white placeholder-gray-400 shadow-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary transition-all" 
                               id="forgot-admin-id" 
                               name="forgot-admin-id" 
                               required 
                               type="text"
                               value="<?php echo isset($_POST['forgot-admin-id']) ? htmlspecialchars($_POST['forgot-admin-id']) : ''; ?>"
                               placeholder="Enter your Admin ID"/>
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="fas fa-id-badge text-gray-400"></i>
                        </div>
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    <div class="flex items-start">
                        ...
                        <div class="text-sm text-blue-700 dark:text-blue-300">
                            ...
                            <ul class="list-disc list-inside space-y-1">
                                <li>Enter your Admin ID and registered email address</li>
    ...
    <script>
        ...
        function openForgotPasswordModal() {
            ...
            setTimeout(() => {
                document.getElementById('forgot-admin-id').focus();
            }, 300);
        }
